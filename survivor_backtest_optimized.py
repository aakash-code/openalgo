...
STRATEGY = {
    ...
    "sl_factor":                5.0,   # Relaxed SL: Exit if price > 5x Entry (less whipsaws)
    "tp_absolute":              1.5,   # TP: Exit if price drops to ₹1.5 (max premium collection)
    "max_multiplier":            20,   # Increased risk cap to allow recovery
}
...
RESULTS_DIR = "survivor_optimized_results"
...
class Position:
    ...
    def __init__(self, *, symbol, option_type, strike, expiry,
        ...
        self.tp_price = STRATEGY["tp_absolute"]
...
class BacktestEngine:
    ...
    def _run_day(self, day: date):
        ...
        for _, row in spot_df.iterrows():
            ...
            # 1. Update P&L on all open positions & Check SL/TP
            to_close = []
            for pos in self.open_positions:
                ...
                if opt_px > 0:
                    ...
                    # Optimization: Check for individual Stop Loss
                    if opt_px >= pos.sl_price:
                        pos.close(opt_px, ts, "STOP-LOSS")
                        to_close.append(pos)
                        print(f"  [{ts.strftime('%H:%M')}] SL EXIT   {pos.symbol:<30} @ {opt_px:>7.2f} (Entry {pos.entry_price:.2f})")
                    
                    # Optimization: Check for individual Take Profit
                    elif opt_px <= pos.tp_price:
                        pos.close(opt_px, ts, "TAKE-PROFIT")
                        to_close.append(pos)
                        print(f"  [{ts.strftime('%H:%M')}] TP EXIT   {pos.symbol:<30} @ {opt_px:>7.2f} (Entry {pos.entry_price:.2f})")
            ...
            if pe_sig:
                if pe_mult > self.scfg["max_multiplier"]:
                    print(f"  [{ts.strftime('%H:%M')}] CAP PE   multiplier {pe_mult} > {self.scfg['max_multiplier']} (Risk Cap)")
            ...
            if ce_sig:
                if ce_mult > self.scfg["max_multiplier"]:
                    print(f"  [{ts.strftime('%H:%M')}] CAP CE   multiplier {ce_mult} > {self.scfg['max_multiplier']} (Risk Cap)")

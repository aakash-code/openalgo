...
STRATEGY = {
    ...
    # --- ULTIMATE OPTIMIZATION (Keep the 10M, Protect the Capital) ---
    "sl_factor":               10.0,   # Ultra-Relaxed SL: Only exit if price > 10x Entry
    "max_multiplier":           100,   # Allow full recovery scaling
}
...
RESULTS_DIR = "survivor_ultimate_results_v2"
...
class DataLoader:
    ...
    def get_previous_close(self, symbol: str, exchange: str, ref_date: date) -> float | None:
        """Fetch the close price of the last trading day before ref_date."""
        ref_ts = int(datetime.combine(ref_date, datetime.min.time()).timestamp())
        query = f"SELECT close FROM market_data WHERE symbol='{symbol}' AND exchange='{exchange}' AND interval='1m' AND timestamp < {ref_ts} ORDER BY timestamp DESC LIMIT 1"
        res = self.conn.execute(query).fetchone()
        val = float(res[0]) if res else None
        print(f"[DEBUG] {ref_date} ref_ts={ref_ts} close={val}")
        return val

    def get_vix_level(self, day: date) -> float:
        """Fetch INDIAVIX close for a given day."""
        start_ts = int(datetime.combine(day, datetime.min.time()).timestamp())
        end_ts   = start_ts + 86400
        res = self.conn.execute(f"""
            SELECT close FROM market_data 
            WHERE symbol='INDIAVIX' AND exchange='NSE_INDEX' AND interval='1m'
            AND timestamp >= {start_ts} AND timestamp < {end_ts}
            ORDER BY timestamp DESC LIMIT 1
        """).fetchone()
        return float(res[0]) if res else 15.0  # Default to 15 if no data
...
class Position:
    ...
    def __init__(self, *, symbol, option_type, strike, expiry,
        ...
        self.sl_price       = entry_price * STRATEGY["sl_factor"]
...
class BacktestEngine:
    ...
    def _run_day(self, day: date):
        ...
        # ── Market Context (Previous Trading Day) ──────────────────────
        p_close = self.loader.get_previous_close('NIFTY', 'NSE_INDEX', day)
        
        # VIX logic: get last known VIX before today's session
        vix = self.loader.get_vix_level(day - timedelta(days=1))
        
        # Dynamic Risk: If VIX > 20, reduce max multiplier to 5x
        dynamic_max_mult = 10 if vix < 20 else 5
        ...
        # Gap Protection Calculation
        opening_price = float(spot_df.iloc[0]["close"])
        gap_pct = 0.0
        if p_close:
            gap_pct = ((opening_price - p_close) / p_close) * 100
        
        # Rule 1: Gap Protection (0.50%)
        skip_new_trades = abs(gap_pct) > 0.50
        ...
        if skip_new_trades:
            print(f"[{day}] GAP PROTECT ({gap_pct:+.2f}%): Skipping all new entries today. VIX: {vix:.2f}")
        else:
            print(
        ...
        for _, row in spot_df.iterrows():
            ...
            # Rule 2: Strict Gamma Protection (13:30)
            is_gamma_zone = is_expiry and time_str >= "13:30"

            # 1. Update P&L on all open positions & Check Ultimate SL
            to_close = []
            for pos in self.open_positions:
                ...
                if opt_px > 0:
                    ...
                    # Ultimate Optimization: Only exit on extreme 10x spikes
                    if opt_px >= pos.sl_price:
                        pos.close(opt_px, ts, "ULTIMATE-SL")
                        to_close.append(pos)
                        print(f"  [{time_str}] SL EXIT   {pos.symbol:<30} @ {opt_px:>7.2f} (Entry {pos.entry_price:.2f})")
            ...
            if pe_sig and not skip_new_trades and not is_gamma_zone:
                # Rule 3: Hard Risk Cap (Dynamic based on VIX)
                if pe_mult > dynamic_max_mult:
                    state.confirm_pe(pe_mult)
                else:
            ...
            if ce_sig and not skip_new_trades and not is_gamma_zone:
                if ce_mult > dynamic_max_mult:
        ...
        # Return today's close to be used for tomorrow's gap calculation
        last_spot_close = float(spot_df.iloc[-1]["close"])
